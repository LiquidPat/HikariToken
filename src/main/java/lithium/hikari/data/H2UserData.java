...
public class H2UserData {
    ...
    public boolean exists(UUID uuid) {
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("SELECT * FROM `user` WHERE (UUID=?)");
    ...
    public void createPlayer(Player p) {
        try {
            if (!exists(p.getUniqueId())) {
                PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement(
                        "INSERT INTO `user` " +
                ...
                statement.setInt(3, HikariTokens.getConfigManager().getDefaultTokens()); // tokens
                statement.setInt(4, HikariTokens.getConfigManager().getDefaultBank()); // bank
    ...
    public static void setIgnore(UUID uuid, boolean ignore) {
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("UPDATE `user` SET Ignore_Pay=? WHERE (UUID=?)");
    ...
    public static void setBank(UUID uuid, int amount) {
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("UPDATE `user` SET Bank=? WHERE (UUID=?)");
    ...
    public static void addBank(UUID uuid, int amount) {
        try {
            PreparedStatement statement = HikariTokens.getMysql().getConnection().prepareStatement("UPDATE `user` SET Bank=? WHERE (UUID=?)");
    ...
    public static void setTokens(UUID uuid, int amount) {
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("UPDATE `user` SET Tokens=? WHERE (UUID=?)");
            statement.setInt(1, amount);
            statement.setString(2, uuid.toString());
            statement.executeUpdate();
        } catch (SQLException e) {
    ...
    public static double getTokensDoubleByName(String player) {
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("SELECT * FROM `user` WHERE (Name=?)");
    ...
    public static TreeMap<String, Integer> getTop10() {
        ...
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("SELECT * FROM `user` ORDER BY 'Tokens' DESC LIMIT " +
                    HikariTokens.getConfigManager().getConfig().getInt("t.plugin.leaderboard-entries"));
            ResultSet resultSet = statement.executeQuery();
    ...
    public static Double getServerTotalTokens() {
        try {
            PreparedStatement statement = HikariTokens.getH2Database().getConnection().prepareStatement("SELECT * FROM `user`");
